!function (a) {
    ...
    a.fn.vectorMap = function (a) {
        ...
        if ("addMap" === a) jvm.Map.maps[arguments[1]] = arguments[2]; else {
...
var jvm = {
    inherits: function (a, b) {
        ...
        var b = new jvm.$.Deferred, c = jvm.$("<img/>");
...
jvm.$ = jQuery, Array.prototype.indexOf || (Array.prototype.indexOf = function (a, b) {
    ...
    setMin: function (a) {
        ...
        var a, b, c = [this.clearMinValue, this.clearMaxValue], d = c[1] - c[0],
            e = Math.pow(10, Math.floor(Math.log(d / 5) / Math.LN10)), f = 5 / d * e, g = [];
    ...
    var a, b, c, d, e = this.series.scale.getTicks(), f = jvm.$("<div/>").addClass("jvectormap-legend-inner");
    ...
    setAttributes: function (a, b) {
        ...
        var b, c, d = -Number.MAX_VALUE, e = Number.MAX_VALUE, f = {};
        if (this.scale instanceof jvm.OrdinalScale || this.scale instanceof jvm.SimpleScale) for (c in a) f[c] = a[c] ? this.scale.getValue(a[c]) : this.elements[c].element.style.initial[this.params.attribute]; else {
            if (void 0 === this.params.min || void 0 === this.params.max) for (c in a) b = parseFloat(a[c]), b > d && (d = b), e > b && (e = b);
    ...
    degRad: 180 / Math.PI, radDeg: Math.PI / 180, radius: 6381372, sgn: function (a) {
        ...
        var d = c * this.radDeg, e = 29.5 * this.radDeg, f = 45.5 * this.radDeg, g = a * this.radDeg,
            h = b * this.radDeg, i = (Math.sin(e) + Math.sin(f)) / 2,
            j = Math.cos(e) * Math.cos(e) + 2 * i * Math.sin(e), k = i * (h - d),
            l = Math.sqrt(j - 2 * i * Math.sin(g)) / i, m = Math.sqrt(j - 2 * i * Math.sin(0)) / i;
        return {x: l * Math.sin(k) * this.radius, y: -(m - l * Math.cos(k)) * this.radius}
    }, aea_inv: function (a, b, c) {
        var d = a / this.radius, e = b / this.radius, f = c * this.radDeg, g = 29.5 * this.radDeg,
            h = 45.5 * this.radDeg, i = (Math.sin(g) + Math.sin(h)) / 2,
            j = Math.cos(g) * Math.cos(g) + 2 * i * Math.sin(g), k = Math.sqrt(j - 2 * i * Math.sin(0)) / i,
            l = Math.sqrt(d * d + (k - e) * (k - e)), m = Math.atan(d / (k - e));
        return {lat: Math.asin((j - l * l * i * i) / (2 * i)) * this.degRad, lng: (f + m / i) * this.degRad}
    }, lcc: function (a, b, c) {
        var d = c * this.radDeg, e = b * this.radDeg, f = 33 * this.radDeg, g = 45 * this.radDeg, h = a * this.radDeg,
            ...
            k = j * Math.pow(1 / Math.tan(Math.PI / 4 + h / 2), i), l = j * Math.pow(1 / Math.tan(Math.PI / 4 + 0), i);
        return {x: k * Math.sin(i * (e - d)) * this.radius, y: -(l - k * Math.cos(i * (e - d))) * this.radius}
    }, lcc_inv: function (a, b, c) {
        var d = a / this.radius, e = b / this.radius, f = c * this.radDeg, g = 33 * this.radDeg, h = 45 * this.radDeg,
            ...
            k = j * Math.pow(1 / Math.tan(Math.PI / 4 + 0), i), l = this.sgn(i) * Math.sqrt(d * d + (k - e) * (k - e)),
            m = Math.atan(d / (k - e));
        return {
    ...
    bindContainerEvents: function () {
        ...
        this.params.panOnDrag && (this.container.mousemove(function (e) {
            ...
            var b = jvm.$(d.container).offset(), c = a.pageX - b.left, e = a.pageY - b.top,
    ...
    bindContainerPointerEvents: function () {
        var a = this, b = new MSGesture, c = this.container[0], d = function (a) {
    ...
    bindElementEvents: function () {
        ...
        this.container.mousemove(function (d) {
            ...
            var b = jvm.$(this).attr("class").baseVal || jvm.$(this).attr("class"),
                ...
                h = jvm.$.Event(c + "TipShow.jvectormap"), i = jvm.$.Event(c + "Over.jvectormap");
    ...
    createTip: function () {
        ...
        this.tip = jvm.$("<div/>").addClass("jvectormap-tip").appendTo(jvm.$("body")), this.container.mousemove(function (b) {
            var c = b.pageX - 15 - a.tipWidth, d = b.pageY - 15 - a.tipHeight;
            5 > c && (c = b.pageX + 15), 5 > d && (d = b.pageY + 15), a.tip.css({left: c, top: d})
        })
    },
    setScale: function (a, b, c, d, e) {
        var f, g, h, i, j, k, l, m, n, o = jvm.$.Event("zoom.jvectormap"), p = this, q = 0,
            r = Math.abs(Math.round(60 * (a - this.scale) / Math.max(a, this.scale))), s = new jvm.$.Deferred;
    ...
    createRegions: function () {
        ...
        }), this.regions[a] = {element: b, config: this.mapData.paths[a]}
    },
    createMarkers: function (a) {
        var b, c, d, e, f, g = this;
        if (this.markersGroup = this.markersGroup || this.canvas.addGroup(), this.markerLabelsGroup = this.markerLabelsGroup || this.canvas.addGroup(), jvm.$.isArray(a)) for (f = a.slice(), a = {}, b = 0; b < f.length; b++) a[b] = f[b];
        for (b in a) e = a[b] instanceof Array ? {latLng: a[b]} : a[b], !1 !== (d = this.getMarkerPosition(e)) && (c = new jvm.Marker({
            map: this,
            style: jvm.$.extend(!0, {}, this.params.markerStyle, {initial: e.style || {}}),
    ...
    latLngToPoint: function (a, b) {
        var c, d, e, f = jvm.Map.maps[this.params.map].projection, g = f.centralMeridian;
    ...
    pointToLatLng: function (a, b) {
        var c, d, e, f, g, h = jvm.Map.maps[this.params.map].projection, i = h.centralMeridian,
    ...
    createSeries: function () {
        ...
        for (b in this.params.series) for (a = 0; a < this.params.series[b].length; a++) this.series[b][a] = new jvm.DataSeries(this.params.series[b][a], this[b], this)
    },
    remove: function () {
    ...
    var b = this;
    this.maps = {}, this.params = jvm.$.extend(!0, {}, jvm.MultiMap.defaultParams, a), this.params.maxLevel = this.params.maxLevel || Number.MAX_VALUE, this.params.main = this.params.main || {}, this.params.main.multiMapLevel = 0, this.history = [this.addMap(this.params.main.map, this.params.main)], this.defaultProjection = this.history[0].mapData.projection.type, this.mapsLoaded = {}, this.params.container.css({position: "relative"}), this.backButton = jvm.$("<div/>").addClass("jvectormap-goback").text("Back").appendTo(this.params.container), this.backButton.hide(), this.backButton.click(function () {
        b.goBack()
    }), this.spinner = jvm.$("<div/>").addClass("jvectormap-spinner").appendTo(this.params.container), this.spinner.hide()
},jvm.MultiMap.prototype = {
    addMap: function (a, b) {
        var c = jvm.$("<div/>").css({width: "100%", height: "100%"});
        return this.params.container.append(c), this.maps[a] = new jvm.Map(jvm.$.extend(b, {container: c})), this.params.maxLevel > b.multiMapLevel && this.maps[a].container.on("regionClick.jvectormap", {scope: this}, function (a, b) {
            var c = a.data.scope, d = c.params.mapNameByCode(b, c);
            c.drillDownPromise && "pending" === c.drillDownPromise.state() || c.drillDown(d, b)
        }), this.maps[a]
    }, downloadMap: function (a) {
        var b = this, c = jvm.$.Deferred();
        ...
        var c = this.history[this.history.length - 1], d = this, e = c.setFocus({region: b, animate: !0}),
        ...
        var a = this.history.pop(), b = this.history[this.history.length - 1], c = this;
        a.setFocus({scale: 1, x: .5, y: .5, animate: !0}).then(function () {
    ...
    insets: [{
        ...
        bbox: [{y: -12671671.123330014, x: -20004297.151525836}, {y: 6930392.02513512, x: 20026572.39474939}],
        left: 0
    }], paths: {
        BD: {
        ...
        _0: {path: "M472.77,172.64l-1.08,-1.29l0.96,-0.77l0.29,-0.83l1.98,1.64l-0.36,0.67l-1.79,0.58Z", name: "Kosovo"},
        JO: {
    ...
    }, height: 440.70631074413296, projection: {type: "mill", centralMeridian: 11.5}, width: 900
});
...
$inputs = array();
//$inputs['allow-tpt-medina-time'] = array("type"=>"time", 'desc'=>'Hotel Time');
$inputs['allow-tpt-medina-driver-contact-number'] = array("type"=>"number", 'desc'=>'Driver Contact Number');
$inputs['allow-tpt-medina-driver-name'] = array("type"=>"text", 'desc'=>'Driver Name');
$inputs['allow-tpt-medina-no-of-seats'] = array("type"=>"number", 'desc'=>'No Of Seats ');
$inputs['allow-tpt-medina-pickup-location'] = array("type"=>"text", 'desc'=>'Pickup Location');
$inputs['allow-tpt-medina-pickup-time'] = array("type"=>"time", 'desc'=>'Pickup Time');
$inputs['allow-tpt-medina-dropoff-location'] = array("type"=>"text", 'desc'=>'Drop off location');
$inputs['allow-tpt-medina-vehicle-number'] = array("type"=>"text", 'desc'=>'Vehicle Number');

echo '<input type="hidden" name="pilgrim_meta[RequestedStatus]" id="RequestedStatus" value="'.$RequestedStatus.'">';
echo '<input type="hidden" id="user-id" name="pilgrim_meta[allow-tpt-medina-user-id]" value=' .$session['id'] . '>';
echo '<input type="hidden" id="requested_status" name="pilgrim_meta[' .$RequestedStatus . '-status]" value = 1 >';


$selects = array();
//$selects['allow-tpt-medina-status'] = array('desc' => 'Status');
$selects['allow-tpt-medina-brn-no'] = array('desc' => 'BRN no/Cash');
...
echo ' <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="Arrival Transport Company">Transport Company</label><br> 
                 <select class="form-control validate[required]" id="check_out_medina_transport_company"  name="pilgrim_meta[allow-tpt-medina-transport-company]">
                  <option value="">Please Select</option>';
...
echo ' <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="Arrival Transport Type">Transport Type</label><br> 
                 <select class="form-control validate[required]" id="allow-tpt-mecca-transport-type"  name="pilgrim_meta[allow-tpt-mecca-transport-type]">
                  <option value="">Please Select</option> 
                   '.$Transports.'
                  ';
...
foreach ($Voucher_pilgrim as $record) {
    ...
    $pilgrimHTML .= ' <tr>        
                              ...
                              <!--  <td>' . ((isset($PilgrimMeta['allow-tpt-medina-time'])) ? TIMEFORMAT($PilgrimMeta['allow-tpt-medina-time']) : '-') . '</td>-->    
                                    <td>' . ((isset($PilgrimMeta['allow-tpt-medina-driver-contact-number'])) ? $PilgrimMeta['allow-tpt-medina-driver-contact-number'] : '-') . '</td>
                                     <td>' . ((isset($PilgrimMeta['allow-tpt-medina-vehicle-number'])) ? $PilgrimMeta['allow-tpt-medina-vehicle-number'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['allow-tpt-medina-driver-name'])) ? $PilgrimMeta['allow-tpt-medina-driver-name'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['allow-tpt-medina-no-of-seats'])) ? $PilgrimMeta['allow-tpt-medina-no-of-seats'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['allow-tpt-medina-transport-company'])) ? OptionName($PilgrimMeta['allow-tpt-medina-transport-company']) : '-') . '</td>
                               </tr>';

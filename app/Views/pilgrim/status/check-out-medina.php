...
$inputs = array();
$inputs['check-out-medina-time'] = array("type"=>"time", 'desc'=>'Hotel Time');
$inputs['check-out-medina-driver-contact-number'] = array("type"=>"number", 'desc'=>'Driver Contact Number','value'=>$finalMeta['allow-tpt-medina-driver-contact-number']);
$inputs['check-out-medina-vehicle-number'] = array("type"=>"text", 'desc'=>'Vehicle Number','value'=>$finalMeta['allow-tpt-medina-vehicle-number']);
$inputs['check-out-medina-driver-name'] = array("type"=>"text", 'desc'=>'Driver Name','value'=> $finalMeta['allow-tpt-medina-driver-name']);
$inputs['check-out-medina-no-of-seats'] = array("type"=>"number", 'desc'=>'No Of Seats ','value'=>$finalMeta['allow-tpt-medina-no-of-seats']);
echo '<input type="hidden" id="user-id" name="pilgrim_meta[check-out-medina-user-id]" value=' .$session['id'] . '>';
echo '<input type="hidden" id="requested_status" name="pilgrim_meta[' .$RequestedStatus . '-status]" value = 1 >';
$inputs['check-out-medina-check-out-date'] = array("type" => "date", 'desc' => 'Check Out Date');
$inputs['check-out-medina-check-out-time'] = array("type" => "time", 'desc' => 'Check Out Time');
$inputs['check-out-medina-departure-time'] = array("type" => "time", 'desc' => 'Departure Time');
$inputs['check-out-medina-transport-destination'] = array("type" => "text", 'desc' => 'Transport Destination');
$inputs['check-out-medina-room-no'] = array("type" => "text", 'desc' => 'Room No');
$inputs['check-out-medina-pickup-point'] = array("type" => "text", 'desc' => 'Pickup Point');

$selects = array();
//$selects['check-out-medina-status'] = array('desc' => 'Status');
$selects['check-out-medina-brn-no'] = array('desc' => 'BRN no/Cash');
$selects['check-out-medina-actual-hotel'] = array('desc' => ' Actual Hotel', 'selected' => 'check-out-medina-actual-hotel');
foreach ($selects as $key => $name) {
    if ($key == 'check-out-medina-brn-no') {
        echo '
        <div class="col-md-3">
    ...
    } else if ($key == 'check-out-medina-actual-hotel') {
        echo '
        <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="' . $name['desc'] . '">' . $name['desc'] . '</label><br>
...
echo ' <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="Arrival Transport Company">Transport Company</label><br> 
                 <select class="form-control validate[required]" id="check_out_medina_transport_company"  name="pilgrim_meta[check-out-medina-transport-company]">
                  <option value="">Please Select</option>';
...
foreach ($Voucher_pilgrim as $record) {
    ...
    $pilgrimHTML .= ' <tr>        
                                   <td> ' . $cnt . '</td> 
                                    ...
                                    <td>' . ((isset($PilgrimMeta['check-out-medina-time'])) ? TIMEFORMAT($PilgrimMeta['check-out-medina-time']) : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['check-out-medina-driver-contact-number'])) ? TIMEFORMAT($PilgrimMeta['check-out-medina-driver-contact-number']) : '-') . '</td>
                                     <td>' . ((isset($PilgrimMeta['check-out-medina-vehicle-number'])) ? $PilgrimMeta['check-out-medina-vehicle-number'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['check-out-medina-driver-name'])) ? $PilgrimMeta['check-out-medina-driver-name'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['check-out-medina-no-of-seats'])) ? $PilgrimMeta['check-out-medina-no-of-seats'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['check-out-medina-transport-company'])) ? OptionName($PilgrimMeta['check-out-medina-transport-company']) : '-') . '</td>
                               </tr>';

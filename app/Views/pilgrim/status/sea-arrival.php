...
$inputs = array();
$inputs['sea-arrival-driver-name'] = array("type" => "text", 'desc' => 'Driver Name','value'=> $finalMeta['allow-tpt-sea-driver-name']);
$inputs['sea-arrival-driver-number'] = array("type" => "number", 'desc' => 'Driver #','value'=> $finalMeta['allow-tpt-sea-driver-contact-number']);
$inputs['sea-arrival-vehicle-number'] = array("type" => "text", 'desc' => 'Vehicle #','value'=> $finalMeta['allow-tpt-sea-vehicle-number']);
$inputs['sea-arrival-actual-no-of-seats'] = array("type" => "number", 'desc' => 'Actual No Of Seats','value'=> $finalMeta['allow-tpt-sea-no-of-seats']);

$inputs['sea-arrival-check-out-date'] = array("type" => "date", 'desc' => 'Check Out Date');
$inputs['sea-arrival-check-out-time'] = array("type" => "time", 'desc' => 'Check Out Time');
$inputs['sea-arrival-departure-time'] = array("type" => "time", 'desc' => 'Departure Time');
$inputs['sea-arrival-transport-destination'] = array("type" => "text", 'desc' => 'Transport Destination');
$inputs['sea-arrival-room-no'] = array("type" => "text", 'desc' => 'Room No');
$inputs['sea-arrival-pickup-point'] = array("type" => "text", 'desc' => 'Pickup Point');


echo '<input type="hidden" id="user_id" name="pilgrim_meta[sea-arrival-user-id]" value=' .$session['id'] . '>';
echo '<input type="hidden" id="requested_status" name="pilgrim_meta[' .$RequestedStatus . '-status]" value = 1 >';

$selects = array();
//$selects['sea_arrival-status'] = array('desc' => 'Status');
$selects['sea-arrival-brn-no'] = array('desc' => ' BRN no/Cash');
$selects['sea-arrival-actual-hotel'] = array('desc' => ' Actual Hotel', 'selected' => 'sea-arrival-actual-hotel');

foreach ($selects as $key => $name) {
    if ($key == 'sea-arrival-brn-no') {
        echo '
        <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="' . $name['desc'] . '">' . $name['desc'] . '</label><br>
                 ...
                 ' . GetBrnList('html', 'transport', $finalMeta['allow-tpt-sea-brn-no']) . '
          </select>
         </div>
        </div>';
    } else if ($key == 'sea-arrival-actual-hotel') {
        echo '
        <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="' . $name['desc'] . '">' . $name['desc'] . '</label><br>
...
echo ' <div class="col-md-4">
            <div class="form-group mb-4">
                <label for="Arrival Transport Company">Transport Company</label><br> 
                 <select class="form-control validate[required]" id="sea_arrival_transport_company"  name="pilgrim_meta[sea-arrival-transport-company]">
                  <option value="">Please Select</option>';
$data['LookupsOptions'] = $Crud->LookupOptions('transportation_company');
foreach ($data['LookupsOptions'] as $options) {
    $selected =(($options['UID'] == $finalMeta['allow-tpt-sea-transport-company']) ? 'selected' : '');
...
foreach ($Voucher_pilgrim as $record) {
    ...
    $pilgrimHTML .= ' <tr>        
                                  <td> ' . $cnt . '</td> 
                                    ...
                                    <td>' . ((isset($PilgrimMeta['sea-arrival-driver-name'])) ? $PilgrimMeta['sea-arrival-driver-name'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['sea-arrival-driver-number'])) ? $PilgrimMeta['sea-arrival-driver-number'] : '-') . '</td>
                                     <td>' . ((isset($PilgrimMeta['sea-arrival-vehicle-number'])) ? $PilgrimMeta['sea-arrival-vehicle-number'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['sea-arrival-actual-no-of-seats'])) ? $PilgrimMeta['sea-arrival-actual-no-of-seats'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['sea-arrival-transport-company'])) ? OptionName($PilgrimMeta['sea-arrival-transport-company']) : '-') . '</td>
 </tr>';

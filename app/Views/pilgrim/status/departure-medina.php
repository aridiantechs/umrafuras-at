...
$inputs = array();
$inputs['departure-medina-hotel-time'] = array("type" => "time", 'desc' => 'Hotel Time');
$inputs['departure-medina-seats'] = array("type" => "text", 'desc' => 'Seats','value'=>$finalMeta['allow-tpt-medina-no-of-seats']);
$inputs['departure-medina-vehicle-number'] = array("type" => "text", 'desc' => 'Vehicle Number','value'=>$finalMeta['allow-tpt-medina-vehicle-number']);
$inputs['departure-medina-driver-name'] = array("type" => "text", 'desc' => 'Driver Name','value'=> $finalMeta['allow-tpt-medina-driver-name']);
$inputs['departure-medina-driver-mobile-number'] = array("type" => "text", 'desc' => 'Driver Mobile Number','value'=>$finalMeta['allow-tpt-medina-driver-contact-number']);

$inputs['departure-medina-check-out-date'] = array("type" => "date", 'desc' => 'Check Out Date');
$inputs['departure-medina-check-out-time'] = array("type" => "time", 'desc' => 'Check Out Time');
$inputs['departure-medina-departure-time'] = array("type" => "time", 'desc' => 'Departure Time');
$inputs['departure-medina-transport-destination'] = array("type" => "text", 'desc' => 'Transport Destination');
$inputs['departure-medina-room-no'] = array("type" => "text", 'desc' => 'Room No');
$inputs['departure-medina-pickup-point'] = array("type" => "text", 'desc' => 'Pickup Point');



echo '<input type="hidden" id="user_id" name="pilgrim_meta[departure-medina-user-id]" value=' . $session['id'] . '>';
echo '<input type="hidden" id="requested_status" name="pilgrim_meta[' . $RequestedStatus . '-status]" value = 1 >';

$selects = array();
$selects['departure-medina-brn-no'] = array('desc' => 'BRN no/Cash');
$selects['departure-medina-actual-hotel'] = array('desc' => ' Actual Hotel', 'selected' => 'departure-medina-actual-hotel');


foreach ($selects as $key => $name) {
    if ($key == 'departure-medina-brn-no') {
        echo '
        <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="' . $name['desc'] . '">' . $name['desc'] . '</label><br>
                 ...
                 ' . GetBrnList('html', 'transport', $finalMeta['departure-medina-brn-no']) . '
          </select>
         </div>
        </div>';
    } else if ($key == 'departure-medina-actual-hotel') {
        echo '
        <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="' . $name['desc'] . '">' . $name['desc'] . '</label><br>
...
echo ' <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="Arrival Transport Company">Transport Company</label><br> 
                 <select class="form-control validate[required]" id="departure_medina_transport_company"  name="pilgrim_meta[departure_medina_transport_company]">
                  <option value="">Please Select</option>';
...
foreach ($Voucher_pilgrim as $record) {
    ...
    $pilgrimHTML .= '<tr>        
                                  <td> ' . $cnt . '</td> 
                                    ...
                                    <td>' . ((isset($PilgrimMeta['departure-medina-hotel-time'])) ? TIMEFORMAT($PilgrimMeta['departure-medina-hotel-time']) : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['departure-medina-seats'])) ? $PilgrimMeta['departure-medina-seats'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['departure-medina-vehicle-number'])) ? $PilgrimMeta['departure-medina-vehicle-number'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['departure-medina-seats'])) ? $PilgrimMeta['departure-medina-seats'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['departure-medina-driver-name'])) ? $PilgrimMeta['departure-medina-driver-name'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['departure-medina-driver-mobile-number'])) ? $PilgrimMeta['departure-medina-driver-mobile-number'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['departure-medina-transport-company'])) ? OptionName($PilgrimMeta['departure-medina-transport-company ']) : '-') . '</td>
                                  </tr>';

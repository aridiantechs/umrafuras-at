...
$inputs = array();
$inputs['departure-other-hotel-time'] = array("type" => "time", 'desc' => 'Hotel Time');
$inputs['departure-other-seats'] = array("type" => "text", 'desc' => 'Seats','value'=>$finalMeta['allow-tpt-other-no-of-seats']);
$inputs['departure-other-vehicle-number'] = array("type" => "text", 'desc' => 'Vehicle Number','value'=>$finalMeta['allow-tpt-other-vehicle-number']);
$inputs['departure-other-driver-name'] = array("type" => "text", 'desc' => 'Driver Name','value'=> $finalMeta['allow-tpt-other-driver-name']);
$inputs['departure-other-driver-mobile-number'] = array("type" => "text", 'desc' => 'Driver Mobile Number','value'=>$finalMeta['allow-tpt-other-driver-contact-number']);
$inputs['departure-other-check-out-date'] = array("type" => "date", 'desc' => 'Check Out Date');
$inputs['departure-other-check-out-time'] = array("type" => "time", 'desc' => 'Check Out Time');
$inputs['departure-other-departure-time'] = array("type" => "time", 'desc' => 'Departure Time');
$inputs['departure-other-transport-destination'] = array("type" => "text", 'desc' => 'Transport Destination');
$inputs['departure-other-room-no'] = array("type" => "text", 'desc' => 'Room No');
$inputs['departure-other-pickup-point'] = array("type" => "text", 'desc' => 'Pickup Point');


echo '<input type="hidden" id="user_id" name="pilgrim_meta[departure-other-user-id]" value=' . $session['id'] . '>';
echo '<input type="hidden" id="requested_status" name="pilgrim_meta[' . $RequestedStatus . '-status]" value = 1 >';

$selects = array();
$selects['departure-other-brn-no'] = array('desc' => 'BRN no/Cash');
$selects['departure-other-actual-hotel'] = array('desc' => ' Actual Hotel', 'selected' => 'departure-other-actual-hotel');
foreach ($selects as $key => $name) {
    if ($key == 'departure-other-brn-no') {
        echo '
        <div class="col-md-3">
    ...
    } else if ($key == 'departure-other-actual-hotel') {
        echo '
        <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="' . $name['desc'] . '">' . $name['desc'] . '</label><br>
...
echo ' <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="Arrival Transport Company">Transport Company</label><br> 
                 <select class="form-control validate[required]" id="departure_other_transport_company"  name="pilgrim_meta[departure_other_transport_company]">
                  <option value="">Please Select</option>';
...
foreach ($Voucher_pilgrim as $record) {
    ...
    $pilgrimHTML .= '<tr>        
                                  <td> ' . $cnt . '</td> 
                                    ...
                                    <td>' . ((isset($PilgrimMeta['departure-other-hotel-time'])) ? TIMEFORMAT($PilgrimMeta['departure-other-hotel-time']) : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['departure-other-seats'])) ? $PilgrimMeta['departure-other-seats'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['departure-other-vehicle-number'])) ? $PilgrimMeta['departure-other-vehicle-number'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['departure-other-seats'])) ? $PilgrimMeta['departure-other-seats'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['departure-other-driver-name'])) ? $PilgrimMeta['departure-other-driver-name'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['departure-other-driver-mobile-number'])) ? $PilgrimMeta['departure-other-driver-mobile-number'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['departure-other-transport-company'])) ? OptionName($PilgrimMeta['departure-other-transport-company ']) : '-') . '</td>
                                  </tr>';

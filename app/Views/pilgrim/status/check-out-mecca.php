...
$inputs = array();
$inputs['check-out-mecca-time'] = array("type"=>"time", 'desc'=>'Hotel Time');
$inputs['check-out-mecca-driver-contact-number'] = array("type"=>"number", 'desc'=>'Driver Contact Number','value'=>$finalMeta['allow-tpt-mecca-driver-contact-number']);
$inputs['check-out-mecca-vehicle-number'] = array("type"=>"text", 'desc'=>'Vehicle Number','value'=>$finalMeta['allow-tpt-mecca-vehicle-number']);
$inputs['check-out-mecca-driver-name'] = array("type"=>"text", 'desc'=>'Driver Name','value'=> $finalMeta['allow-tpt-mecca-driver-name']);
$inputs['check-out-mecca-no-of-seats'] = array("type"=>"number", 'desc'=>'No Of Seats ','value'=>$finalMeta['allow-tpt-mecca-no-of-seats']);
echo '<input type="hidden" id="user_id" name="pilgrim_meta[check-out-mecca-user-id]" value=' .$session['id'] . '>';
echo '<input type="hidden" id="requested_status" name="pilgrim_meta[' .$RequestedStatus . '-status]" value = 1 >';
$inputs['check-out-mecca-check-out-date'] = array("type" => "date", 'desc' => 'Check Out Date');
$inputs['check-out-mecca-check-out-time'] = array("type" => "time", 'desc' => 'Check Out Time');
$inputs['check-out-mecca-departure-time'] = array("type" => "time", 'desc' => 'Departure Time');
$inputs['check-out-mecca-transport-destination'] = array("type" => "text", 'desc' => 'Transport Destination');
$inputs['check-out-mecca-room-no'] = array("type" => "text", 'desc' => 'Room No');
$inputs['check-out-mecca-pickup-point'] = array("type" => "text", 'desc' => 'Pickup Point');

$selects = array();
//$selects['check_out-mecca-status'] = array('desc' => 'Status');
$selects['check-out-mecca-brn-no'] = array('desc' => 'BRN no/Cash');
$selects['check-out-mecca-actual-hotel'] = array('desc' => ' Actual Hotel', 'selected' => 'check-out-mecca-actual-hotel');
foreach ($selects as $key => $name) {
    if ($key == 'check-out-mecca-brn-no') {
        echo '
        <div class="col-md-3">
    ...
    } else if ($key == 'check-out-mecca-actual-hotel') {
        echo '
        <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="' . $name['desc'] . '">' . $name['desc'] . '</label><br>
...
echo ' <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="Arrival Transport Company">Transport Company</label><br> 
                 <select class="form-control validate[required]" id="check_out_mecca_transport_company"  name="pilgrim_meta[check-out-mecca-transport-company]">
                  <option value="">Please Select</option>';
...
foreach ($Voucher_pilgrim as $record) {
    ...
    $pilgrimHTML .= ' <tr>     
                                    ...
                                    <td>' . ((isset($PilgrimMeta['check-out-mecca-time'])) ? TIMEFORMAT($PilgrimMeta['check-out-mecca-time']) : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['check-out-mecca-driver-contact-number'])) ? TIMEFORMAT($PilgrimMeta['check-out-mecca-driver-contact-number']) : '-') . '</td>
                                     <td>' . ((isset($PilgrimMeta['check-out-mecca-vehicle-number'])) ? $PilgrimMeta['check-out-mecca-vehicle-number'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['check-out-mecca-driver-name'])) ? $PilgrimMeta['check-out-mecca-driver-name'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['check-out-mecca-no-of-seats'])) ? $PilgrimMeta['check-out-mecca-no-of-seats'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['check-out-mecca-transport-company'])) ? OptionName($PilgrimMeta['check-out-mecca-transport-company']) : '-') . '</td>
                               </tr>';

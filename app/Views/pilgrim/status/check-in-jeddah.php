...
$inputs = array();
$inputs['check-in-jeddah-room-no'] = array("type" => "text", 'desc' => ' Room Number','value'=>$finalMeta['allow-htl-jeddah-room-no']);
$inputs['check-in-jeddah-no-of-bed'] = array("type" => "number", 'desc' => ' Actual No Of Bed (Room)','value'=>$finalMeta['allow-htl-jeddah-no-of-bed']);
$inputs['check-in-jeddah-no-of-bed-voucher'] = array("type" => "number", 'desc' => 'Actual No Of Bed (Voucher)','value'=>$finalMeta['allow-htl-jeddah-no-of-bed-voucher']);
$inputs['check-in-jeddah-in-date'] = array("type" => "date", 'desc' => ' Check In Date','value'=>$finalMeta['allow-htl-jeddah-in-date']);
$inputs['check-in-jeddah-actual-arrival-time'] = array("type" => "time", 'desc' => ' Actual Arrival Time','value'=>$finalMeta['allow-htl-jeddah-actual-arrival-time']);

$inputs['check-in-jeddah-out-date'] = array("type" => "date", 'desc' => ' Check Out Date','value'=>$finalMeta['allow-htl-jeddah-out-date']);
echo '<input type="hidden" id="user_id" name="pilgrim_meta[check-in-jeddah-user-id]" value=' . $session['id'] . '>';
echo '<input type="hidden" id="requested_status" name="pilgrim_meta[' . $RequestedStatus . '-status]" value = 1 >';

$selects = array();
$selects['check-in-jeddah-brn-no'] = array('desc' => ' BRN no/Cash');
        ...
        <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="' . $name['desc'] . '">' . $name['desc'] . '</label><br>
                 ...
                 ' . GetBrnList('html', 'hotel', $finalMeta['allow-htl-jeddah-brn-no']) . '
          </select>
         </div>
        </div>';
}
$Hotelselect = array();
$Hotelselect['check-in-jeddah-package-Hotel'] = array('desc' => ' Package Hotel','selected'=>'allow-htl-jeddah-package-Hotel');
$Hotelselect['check-in-jeddah-actual-Hotel'] = array('desc' => ' Actual Hotel','selected'=>'allow-htl-jeddah-actual-Hotel');
$Hotelselect['check-in-jeddah-accommodation'] = array('desc' => ' Accommodation','selected'=>'allow-htl-jeddah-accommodation');
...
foreach ($Hotelselect as $key => $name) {
    echo '         
        <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="' . $name['desc'] . '">' . $name['desc'] . '</label><br>';
    if ($key == 'check-in-jeddah-package-Hotel') {
    ...
    if ($key == 'check-in-jeddah-accommodation') {
        {
    ...
    if ($key == 'check-in-jeddah-actual-Hotel') {
        echo '
...
foreach ($inputs as $key => $name) {
    echo '
        <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="' . $name['desc'] . '">' . $name['desc'] . '</label><br>';
    if ($key == 'check-in-jeddah-actual-arrival-time') {
        ...
        echo ' placeholder="' . $name['desc'] . '"   ' . (($key == "check-in-jeddah-in-date") ? "min = $arrival " . "value = " . $arrival . " " . "max = $return" : "") . ' ' . (($key == "check-in-jeddah-out-date") ? "min = $arrival " . "value = " . $return . " " . "max = $return" : "") . ' value=' . ((isset($name['value'])) ? $name['value'] : '') . ' >';
    }
...
foreach ($Voucher_pilgrim as $record) {
    ...
    $PackageHotelName = $Crud->SingleRecord('packages."Hotels"', array("UID" => $PilgrimMeta['check-in-jeddah-package-Hotel']));
    $ActualHotelName = $Crud->SingleRecord('packages."Hotels"', array("UID" => $PilgrimMeta['check-in-jeddah-package-Hotel']));
    ...
    $pilgrimHTML .= ' <tr>        
                                ...
                                <td>' . ((isset($PilgrimMeta['check-in-jeddah-accommodation'])) ? $PilgrimMeta['check-in-jeddah-accommodation'] : '-') . '</td>
                                <td>' . ((isset($PilgrimMeta['check-in-jeddah-actual-arrival-time'])) ? $PilgrimMeta['check-in-jeddah-actual-arrival-time'] : '-') . '</td>
                                <td>' . ((isset($PilgrimMeta['check-in-jeddah-room-no'])) ? $PilgrimMeta['check-in-jeddah-room-no'] : '-') . '</td>
                                <td>' . ((isset($PilgrimMeta['check-in-jeddah-no-of-bed'])) ? $PilgrimMeta['check-in-jeddah-no-of-bed'] : '-') . '</td>
                                <td>' . ((isset($PilgrimMeta['check-in-jeddah-in-date'])) ? DATEFORMAT($PilgrimMeta['check-in-jeddah-in-date']) : '-') . '</td>
                                <td>' . ((isset($PilgrimMeta['check-in-jeddah-out-date'])) ? DATEFORMAT($PilgrimMeta['check-in-jeddah-out-date']) : '-') . '</td>
                                 </tr>';

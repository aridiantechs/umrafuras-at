...
$inputs = array();
$inputs['other-arrival-driver-name'] = array("type" => "text", 'desc' => 'Driver Name','value'=> $finalMeta['allow-tpt-other-driver-name']);
$inputs['other-arrival-driver-number'] = array("type" => "number", 'desc' => 'Driver #','value'=>$finalMeta['allow-tpt-other-driver-contact-number']);
$inputs['other-arrival-vehicle-number'] = array("type" => "text", 'desc' => 'Vehicle #','value'=>$finalMeta['allow-tpt-other-vehicle-number']);
$inputs['other-arrival-actual-no-of-seats'] = array("type" => "number", 'desc' => 'Actual No Of Seats','value'=>$finalMeta['allow-tpt-other-no-of-seats']);


$inputs['other-arrival-check-out-date'] = array("type" => "date", 'desc' => 'Check Out Date');
$inputs['other-arrival-check-out-time'] = array("type" => "time", 'desc' => 'Check Out Time');
$inputs['other-arrival-departure-time'] = array("type" => "time", 'desc' => 'Departure Time');
$inputs['other-arrival-transport-destination'] = array("type" => "text", 'desc' => 'Transport Destination');
$inputs['other-arrival-room-no'] = array("type" => "text", 'desc' => 'Room No');
$inputs['other-arrival-pickup-point'] = array("type" => "text", 'desc' => 'Pickup Point');


echo '<input type="hidden" id="user_id" name="pilgrim_meta[other-arrival-user-id]" value=' .$session['id'] . '>';
echo '<input type="hidden" id="requested_status" name="pilgrim_meta[' .$RequestedStatus . '-status]" value = 1 >';

$selects = array();
//$selects['other_arrival-status'] = array('desc' => 'Status');
$selects['other-arrival-brn-no'] = array('desc' => 'BRN no/Cash');
$selects['other-arrival-actual-hotel'] = array('desc' => ' Actual Hotel', 'selected' => 'other-arrival-actual-hotel');

foreach ($selects as $key => $name) {
    if ($key == 'other-arrival-brn-no') {
        echo '
        <div class="col-md-3">
    ...
    } else if ($key == 'other-arrival-actual-hotel') {
        echo '
        <div class="col-md-3">
            <div class="form-group mb-4">
                <label for="' . $name['desc'] . '">' . $name['desc'] . '</label><br>
...
echo ' <div class="col-md-4">
            <div class="form-group mb-4">
                <label for="Arrival Transport Company">Transport Company</label><br> 
                 <select class="form-control validate[required]" id="other_arrival_transport_company"  name="pilgrim_meta[other-arrival-transport-company]">
...
foreach ($Voucher_pilgrim as $record) {
    ...
    $pilgrimHTML .= '<tr>        
                                <td> ' . $cnt . '</td> 
                                    ...
                                    <td>' . ((isset($PilgrimMeta['other-arrival-driver-name'])) ? $PilgrimMeta['other-arrival-driver-name'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['other-arrival-driver-number'])) ? $PilgrimMeta['other-arrival-driver-number'] : '-') . '</td>
                                     <td>' . ((isset($PilgrimMeta['other-arrival-vehicle-number'])) ? $PilgrimMeta['other-arrival-vehicle-number'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['other-arrival-actual-no-of-seats'])) ? $PilgrimMeta['other-arrival-actual-no-of-seats'] : '-') . '</td>
                                    <td>' . ((isset($PilgrimMeta['other-arrival-brn-no'])) ? $PilgrimMeta['other-arrival-brn-no'] : '-') . '</td>
                              <td>' . ((isset($PilgrimMeta['other-arrival-transport-company'])) ? OptionName($PilgrimMeta['other-transport-company']) : '-') . '</td>

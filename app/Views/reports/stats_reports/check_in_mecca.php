...
$session = session();
$SessionFilters = $session->get('CheckInMeccaSessionFilters');
    ...
    <div class="layout-px-spacing">
        <div class="row layout-top-spacing">
            <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                <h4 class="page-head">Check In Mecca
                    <?php if ($CheckAccess['umrah_reports_status_stats_export_check_in_mecca']) { ?>
                    <a type="button" class="btn btn_customized  btn-sm float-right"
                       onclick="" href="<?= $path ?>exports/check_in_mecca" target="_blank">Export Records
                    </a>
                    <?php  } ?>
                </h4>
            </div>
            <div class="col-xl-12 col-lg-12 col-md-12 layout-spacing">
                <form onsubmit="CheckInMeccaFiltersFormSubmit('CheckInMeccaSearchFiltersForm'); return false;"
                      class="section contact" id="CheckInMeccaSearchFiltersForm">
                    <input type="hidden" name="SessionKey" id="SessionKey" value="CheckInMeccaSessionFilters">
                    <input type="hidden" name="check_in_date_from" id="check_in_date_from" value="<?=$CheckInDateFrom?>">
                            ...
                            <div id="FilterDetails" class="collapse show" aria-labelledby=""
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-12" id="CheckInMeccaAjaxResult"></div>
                                        <div class="col-md-12" id="FilterButtons">
                                            <div class="form-group float-right">
                                                <button onclick="CheckInMeccaFiltersFormSubmit('CheckInMeccaSearchFiltersForm');"
                                                        id="btnsearch" type="button" class="btn btn-success">
                                                    Search
                                                </button>
                                                <button onclick="ClearSession('CheckInMeccaSessionFilters');"
                    ...
                    <div class="table-responsive mb-4 mt-4">
                        <table id="ReportTable" class="table table-hover non-hover display nowrap" style="width:100%">
                            ...
                            foreach ($records as $record) {
                                ...
                                //echo '<pre>';print_r($PilgrimMetaRecords);
                                //echo '<pre>'.$record['LeaderPilgrimUID'];
...
<script type="application/javascript">
    ...
    function CheckInMeccaFiltersFormSubmit(parent) {

        var data = $("form#" + parent).serialize();
        var rslt = AjaxResponse('filters_session/create_filters_session', data);
        if (rslt.status == 'success') {

            GridMessages(parent, 'CheckInMeccaAjaxResult', 'alert-success', rslt.msg, 250);
            setTimeout(function () {
                location.reload();
            }, 500);
        }
    ...
    function ClearSession(SessionKey) {
        ...
        if (rslt.status == 'success') {
            ...
            GridMessages('CheckInMeccaSearchFiltersForm', 'CheckInMeccaAjaxResult', 'alert-success', rslt.msg, 250);
            setTimeout(function () {
                location.reload();
            }, 500);
        }
    ...
    <?php if ($CheckAccess['umrah_reports_status_stats_export_check_in_mecca']) { ?>
    setTimeout(function () {
        $('<a href="<?=$path?>exports/check_in_mecca" target="_blank" class="btn btn_customized  btn-sm float-right">Export Record</a>').appendTo(".table-responsive");
    }, 100);
    <?php } ?>
</script>

    ...
    <div class="layout-px-spacing">
        <div class="row layout-top-spacing">
            <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                <h4 class="page-head">B2B Package <small>(External Agent)</small>
                    <?php
                    if ($CheckAccess['umrah_services_packages_b2b_external_add']) {
                        $Crud = new Crud();
                        $Page = getSegment(2);
                        ...
                        if (count($records) > 0) {
                            ...
                            echo '   <a class="btn btn_customized btn-sm float-right" href="' . $path . 'package/new_package_for_external_agent">Add New</a>';
                        }
                    } ?>
                    <?php if ($CheckAccess['umrah_services_packages_b2b_external_copy_package']) { ?>
                    ...
                    <div class="table-responsive mb-4 mt-4 datatableparentdiv">
                        <table id="MainRecords" class="table table-hover non-hover display nowrap" style="width:100%">
                            ...
                            foreach ($packages as $record) {
                                ...
                                if ($CheckAccess['umrah_services_packages_b2b_external_Update']) {
                                    $actions .= '<a class="dropdown-item" href="' . $path . 'package/edit_package_for_external_agent/' . $record['UID'] . '">Update</a>';
                                }
                                if ($CheckAccess['umrah_services_packages_b2b_external_download']) {
                            ...
                            if ($CheckAccess['umrah_services_packages_b2b_external_Update'] || $CheckAccess['umrah_services_packages_b2b_external_download']) {
                                echo ' 
                                    <td>
                                        <div class="btn-group">
                                            <button type="button"
...
<script type="application/javascript">
    ...
    <?php if ($CheckAccess['umrah_services_packages_b2b_external_export']) { ?>
    setTimeout(function () {
        $('<a target="_blank" href="<?=$path?>exports/b2b_external_agent_package_list" class="dt-filter-btn">Export</a>').appendTo(".dataTables_wrapper .dataTables_filter");
    }, 100);
    <?php } ?>
</script>
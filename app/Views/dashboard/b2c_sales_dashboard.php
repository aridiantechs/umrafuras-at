...
$total_leads = $sales->GetTotalLeads($DomainID,'B2C');
$fresh_leads = $sales->GetFreshLeads($DomainID,'B2C');

$un_assign_leads = $sales->GetUnAssignLeads($DomainID,'B2C');
$today_followup_leads = $sales->GetTodayFollowUpLeads($DomainID,'B2C');
$pending_followup_leads = $sales->GetPendingFollowUpLeads($DomainID,'B2C');
$upcoming_followup_leads = $sales->GetUpComingFollowUpLeads($DomainID,'B2C');


$count_leads_status = $sales->CountLeadStatus($DomainID,'B2C');
$count_leads_products = $sales->CountLeadProducts($DomainID,'B2C');
//echo "<pre>";
                    ...
                    foreach ($Products as $value) {
                        if ($value == 'home' || $value == 'visitor' || $value == 'sales' || $value == 'hr') {
                            ...
                            <div class="<?= $cnt ?> tab-pane fade show <?= (($cnt == 1) ? 'active' : '') ?>"
                                ...
                                <div class="row layout-top-spacing">
                                    <?php foreach ($B2CLeadStatusArray as $key => $valueStatus) {
                                        if ($key != 'new') {
                                            $produceBased_leads_status = $sales->GetProductBasedLeadStatus($DomainID, $value, $key ,'B2C'); ?>
                                            <div class="col-xl-4 col-lg-6 col-md-6 col-sm-6 col-12 layout-spacing">
                                                <div class="widget widget-card-four">
                                                    <div class="widget-content">
                                                        <div class="w-content">
